           ...
           if z<STARTING_PIXEL_BEAMFORMING
               ...
               for Rx = 1:ELEMENT_NUMBER
                   ...
                   if theta_rad>=0
                        ...
                        ttx = 1/c0*(z*dz*cos(abs(theta_rad))+((ELEMENT_NUMBER-1)*pitch_grids*dx-x*dx)*sin(abs(theta_rad)))/dt;%-3*(1/trans_freq)/dt;%+250;%;
                   end
...
save([dir_to_save '_RF_beamformed_win2p5degrees_compounded_at_Tx_' num2str(round(Tx_angle_degree,3)*1000) '_Rx_'  num2str(round(Rx_angle_degree,3)*1000)], 'compunded_before_b_mode');

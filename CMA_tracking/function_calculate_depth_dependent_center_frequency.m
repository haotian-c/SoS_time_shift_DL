...
freq_all_A_lines = [];
window_length = 16;  % Try 256 or 512
half_win = floor(window_length/2);
left_right_win = 20;
for column_i =left_right_win+1:size(rf_data,2)-left_right_win-1
signal = mean(rf_data(:, column_i-left_right_win:column_i+left_right_win), 2);
...
figure();imagesc(freq_all_A_lines);clim([3.5e6,5.5e6]);title('single line')

mean_freq_line = mean(freq_all_A_lines,2);
figure();plot(mean_freq_line);title('averaged line')

window_size = 91;  % Make this much larger than SG window (must be odd)
smoothed_mean_freq_line = movmean(mean_freq_line, window_size, 'omitnan');
figure();plot(smoothed_mean_freq_line);title('averaged line')

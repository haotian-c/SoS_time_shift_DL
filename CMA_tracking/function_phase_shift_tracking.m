function function_phase_shift_tracking(dir_binary_data, source_dir_prefix, target_dir_prefix, ...
    ...
    for psf_direction_idx = 1:length(psf_dict)
        ...
        target_filename = [target_dir_prefix '_psf_' psf_direction_i 'compounded_phase_shift' ...
            '_kernel_size_z_' int2str(PARAM_tracking_kernel_z) '_kernel_size_y_' int2str(PARAM_tracking_kernel_y)];

        phase_shift_map = zeros(size(load([source_dir_prefix ...
            '_RF_beamformed_win2p5degrees_compounded_at_Tx_' int2str(0*1000) '_Rx_' int2str(0*1000) '.mat']).compunded_before_b_mode));
        ...
        for entry_i = 1:length(phase_track_struct.(psf_direction_i))
            ...
            RF_steering_angle_A = load([source_dir_prefix ...
                '_RF_beamformed_win2p5degrees_compounded_at_Tx_' int2str(tx_before_steer*1000) ...
                '_Rx_' int2str(rx_before_steer*1000) '.mat']).compunded_before_b_mode;
            RF_steering_angle_B = load([source_dir_prefix ...
                '_RF_beamformed_win2p5degrees_compounded_at_Tx_' int2str(tx_after_steer*1000) ...
                '_Rx_' int2str(rx_after_steer*1000) '.mat']).compunded_before_b_mode;

...
def add_noise(batch_data):
    """
    Apply noise to data to improve training.
    Args:
        batch_data (torch.Tensor): Input batch of shape [B, C, H, W]
    Returns:
        torch.Tensor: Batch data with noise
    """
    # Additive Gaussian noise
    noise_mask = np.ones([batch_data.shape[2], batch_data.shape[3]])
    batch_data = batch_data + noise_mask * np.random.normal(0, 1, batch_data.shape) * 0.5

    # Subtractive random Gaussian mask
    from utils import create_rand_gaussian_mask  # If not already imported at top
    batch_data -= torch.tensor(create_rand_gaussian_mask(batch_data.shape[-2:], 50)) * batch_data

    # Random amplification
    row_indices = np.random.randint(1, batch_data.shape[2] - 1, size=len(batch_data))
